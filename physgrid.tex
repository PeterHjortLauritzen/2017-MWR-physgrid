...
\abstract{Atmospheric modeling with element-based high-order Galerkin methods presents a unique challenge to the conventional physics-dynamics coupling paradigm, in which the physical parameterizations (`physics') are evaluated on the dynamical core grid, due to highly irregular spacing of the nodal solutions within an element. In addition, evaluating the physics at the nodal points exacerbates numerical noise due to the Galerkin method, enabling and amplifying local extrema at element boundaries. Grid imprinting may be substantially reduced through the introduction of an entirely separate, approximately isotropic finite-volume grid for evaluating the physics forcing. Integration of the spectral basis over the approximately isotropic control-volumes provides an area average state to the physics, which is more representative of the state in the vicinitny of the nodal values, rather than the nodal value itself. The quasi-equal area physics grid is implemented in NCAR's Community Atmosphere Model with Spectral Elements (CAM-SE), and is shown to be effective at mitigating grid imprinting in the solution. Further, the finite-volume physics grid ensures more consistent coupling with other componenents of the Community Earth System Model, since one can be assured that the fluxes on the physics grid are infact, volume-average fluxes.}
