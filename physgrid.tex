...
\begin{figure*}[t]
\begin{center}
%\noindent\includegraphics[width=37pc,angle=0]{temp_pdf_200bins_30min_trX25min.pdf}\\
%\noindent\includegraphics[width=18pc,angle=0]{figs/temp_pdf_200bins_30min_trX25min.pdf}\\
\noindent\includegraphics[width=37pc,angle=0]{figs/temp_pdf_200bins_30min_trX25min.pdf}\\
\end{center}
...
{\color{red}{The global mean and variance of the low-level physics tendencies are slightly lower in $pg3$ compared with $np4$, on the GLL grid (by about 1\% and 6\% for the mean and variance, respectively; Figure~\ref{fig:tendency_imprint}). Through re-creating Figure \ref{fig:omega-se-volumes}a, but using the $\omega$ field on the GLL grid in the $pg3$ simulations, the frequency of large magnitude $\omega$ values (less than -1.0 Pa/s) associated with interior, corner and edge nodes are slightly lower (not shown). This suggests that the lower magnitude physics forcing in $pg3$ impacts the state on the GLL grid, albeit modestly. Therefore the lower frequency of large magnitude $\omega$ in the $pg3$ run (Figure \ref{fig:omega-se-volumes}) is not solely due to the smoothing effect of integrating the basis functions over control volumes, but also a result of lower magnitude physics tendencies feeding back onto the dynamical state.}}
