...
In the aqua-planets, the magnitude of grid imprinting is actually quite modest and difficult to observe in the mean. Grid imprinting associated with the flow around obstacles, however, is more problematic. In order to diagnose grid imprinting associated with topographic flow, an idealized held-suarez conifugration \citep{HS1994} is outfitted with real world topography, and ran for a year using the $ne30np4$ and $ne30pg3$ configurations. Figure~\ref{} shows the mean $omega$ at two different vertical levels in the middle troposphere. At higher latitudes (such as southern Andes), the flow is smooth, conforming reasonably to the underlying topography. At lower latitudes, over the Andes or the Himlayas, their are notable upward-downward couplets, with strong upward motion typically occurring near the foot of a mountain and coinciding with an element boundary, and downward compensating motion about two GLL nodes downstream. The vertical structure of $omega$ in regions of strong grid-imprinting is depicted in Figure~\ref{fig:}, which are great-circle distance-pressure transects over the Andes and Himalayas., the transects provides in \ref{fig:}. of $omega$ The upward-downward couplets tend to occur in regions of strong gravitational instability in the tropics, and reflect the release of instabilty due to flow being forced up a mountain. 

The grid-imprinting and up-down couplets are exaggerated in the modified held-suarez configuration, compared to more realistic configurations using a complete physical parameterization package. Figure~\ref{fig:} shows contours of the $omega$ field in a more realistic, AMIP-type simulation. First, note that the implications of this finding are that one can accept a level of grid-impriniting in the modified Held-Siarez configuration, at trust it will be abest in more real-world configurations. 
