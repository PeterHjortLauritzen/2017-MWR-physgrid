...
{\color{red}The physics and dynamics are integrated in time using a sequential-update approach \citep{LetAl2018JAMES}. The dynamical core is sub-cycled over the (usually) longer physics time-step, $\Delta t_{phys}$, e.g., the vertical remapping time-step $\Delta t_{remap}$ is cycled $rsplit$ times, summing to $\Delta t_{phys}$. In CAM-SE, a fraction of the physics forcing, e.g., $f_q * \Delta t_{remap}$ is applied at the beginning of each $rsplit$ vertical remap subcycles, such that the full forcing ($f_q * \Delta t_{phys}$) is realized over the course of a physics time-step. This approach of dribbling the tendencies over sub-intervals has the advantage of reducing gravity wave noise \citep{TJ2016GMD}, but may disrupt tracer mass conservation \citep{}. In CAM-SE-CSLAM, all but the tracer mass quantaties are dribbled, with tracer mass receiving the full physics update, e.g., $f_q * \Delta t_{remap}$, applied only at the beginning of the first remap sub-cycle, and thereby conserving tracer mass.{}}

\section{Results}\label{sec:results}

{\color{red}A hierarchy of idealized model configurations are presented in order to elucidate the differences between CAM-SE and CAM-SE-CSLAM (CAM-trunk revision $88685$). Here, the configurations are presented in order of increasing complexity, each with a pair of approximately $1^{\circ}$ simulations, pertaining to the $ne30np4$ and $ne30pg3$ grids.{}}

\subsection{{\color{red}Moist Baroclinic Wave{}}}

{\color{red}The moist baroclinic wave test case was included from the `CESM Simple Models' project \citep{CESM_SIMPLER_MODELS}, as part of the release of CESM2. It is effectively the dry test-case of \cite{UMJS2014QJRMS}, but initialized with moisture and coupled to the Kessler moist physics routine. A measure of the uncertainty of the reference solution, the $L_2$ difference norm between two high-resolution solutions using different dynamical cores, was presented in \cite{LetAl2018JAMES} and provided again here in Figure \ref{fig:norm}. The $L_2$ norm between CAM-SE and CAM-SE-CSLAM lies below the uncertainty of the reference solution, indicating their differences are insignificant. 

\begin{figure}[t]
\noindent\includegraphics[width=15pc,angle=0]{figs/temp_l2.pdf}\\
\caption{$L_2$ difference norms of $p_s$ in the moist baroclinic wave simulations. $L_2$ values lying within the yellow region fall below the estimate of the uncertainty in the reference solution (black curve).}
\label{fig:norm}
\end{figure}

The flow field of the baroclinic wave test is used to drive the terminator "toy"-chemistry test of \cite{LCLVT2015GMD}. The terminator test is used to assess the tracer mass conservation properties in models through advecting a reactive species across the terminator line. The model is intialized with a specified, constant tracer mass everywhere ($4e-6 kg/kg$), such that if tracer mass is conserved, then the column integrated sum of the species should not vary in time. Figure \ref{fig:term} provides a snapshot of the mass-weighted, vertically integrated tracer mass at day 15. In CAM-SE, the tracer mass is not conserved by day 15 and the field is populated by overshoots and undershoots. In contrast, by day 15, CAM-SE-CSLAM still conserves tracer mass to within machine precision, consistent with the previous results of this test-case initialized with a dry baroclinic wave \citep{LTOUNGK2017MWR}.{}} 
...
Two year long aqua-planet simulations are performed using CAM-SE and CAM-SE-CSLAM. A plot similar to Figure \ref{fig:omega-se-volumes}a is constructed for the CAM-SE-CSLAM simulation, a probability density distribution of upward $\omega$ conditionally sampled based on location within the element. Similar to Figure \ref{fig:omega-se-volumes}a, Figure \ref{fig:omega-se-volumes}b divides up the control volumes by corner, edge and interior cells. Through the use of the quasi-equal area physics grid, the dynamical core state appears more or less independent of location within the element, a marked improvement over CAM-SE. Since the state is approximately independent of in-element location, it follows that the physics forcing, which is evaluated from the dynamical core state, may be expected to also show an improvment in grid-imprinting. 

In contrast to the differences between the two model solutions in Figure \ref{fig:omega-se-volumes}, away from the grid-scale, the mean states are very similar. Figure~\ref{fig:zonal} shows the zonal-mean climatological precipitation rates in CAM-SE and CAM-SE-CSLAM. Considering how sensitive this aqua-planet configuration is to design choices in CAM-SE \citep{LetAl2018JAMES}, it is somewhat unexpected that the zonal means look so similar to one another. 

The low-level, mean and variance of the physics tendencies in the two aqua-planet simulations are shown in Figure \ref{fig:tendency_imprint}. The mean states in the two models resemble one another, consistent with the similar climatological zonal mean precipitation rate in the simulations (not shown). The mean physics tendencies contains modest grid imprinting in the $ne30np4$ configuration (barely visisble near the storm-track regions), while in the variance field, grid imprinting is both ubiquitous and unmistakable. The variance is larger on boundary nodes, manifesting as a `stitching' pattern resembling the cube-sphere grid. In $ne30pg3$, the grid imprinting is all but eliminated based on the mean and variance of the physics tendencies (Figure~\ref{fig:tendency_imprint}), consistent with our expectation.
...
\noindent\includegraphics[width=15pc,angle=0]{figs/QPC4_bar_skk.pdf}\\
